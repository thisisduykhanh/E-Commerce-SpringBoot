...
const columns = [
    ...
    {
        formatter: (row) => {
            return new Intl.NumberFormat('vi-VN').format(row.visitors);
        },
        name: 'Visitors',
        width: '150px',
    },
    {
        formatter: (row) => {
            return new Intl.NumberFormat('vi-VN').format(row.uniqueVisits);
        },
        name: 'Unique Visits',
        width: '150px',
    },
    {
        formatter: (row) => {
            return new Intl.NumberFormat('vi-VN', { style: 'percent', maximumFractionDigits: 2 }).format(
                row.bounceRate / 100
            );
        },
        name: 'Bounce Rate',
