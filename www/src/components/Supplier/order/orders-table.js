...
export function OrdersTable({ rows, status }) {
    ...
    const columns = [
        {
            formatter: (row) => (
                <Stack direction="row" spacing={2} sx={{ alignItems: 'center' }}>
                    ...
                    >
                        <Typography variant="caption">{dayjs(row.createDate).format('MMM').toUpperCase()}</Typography>
                        <Typography variant="h6">{dayjs(row.createDate).format('D')}</Typography>
                    </Box>
                    <div>
                        <Link
                            color="text.primary"
        ...
        },
        {
            formatter: (row) => {
                ...
                //     rejected: { label: 'Rejected', icon: <MinusIcon color="var(--mui-palette-error-main)" /> },
                //   };
                const { label, icon } = mapping[row.orderStatus] ?? { label: 'Từ chối', icon: null };
                        ...
                        <>
                            <Button
                                sx={{
                                    ...
                                    padding: '6px 16px',
                                    '&:hover': {
                                        backgroundColor: '#5148cc',
                                    },
                                }}
                                onClick={() => handleClick(row.id, 3)}
                            >
                                Duyệt
                            </Button>
                            <Button
    ...
    async function handleClick(idOrder, idStatus) {
        try {
            const response = await updateOrderStatus(idOrder, idStatus);
            logger.debug('update orders:', response.data.content);
